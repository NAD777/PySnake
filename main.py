...
class Snake:
    ...
    def is_alive(self):
        for el in self.tail[1:]:
            if self.x_head == el.get_x() and self.y_head == el.get_y():
                return False
...
class Board:
    def __init__(self, snake):
        ...
        self.snake = snake
        self.score = 0
        self.fruit = Fruit(randint(0, WIDTH - 1) // 10 * 10,
                           randint(0, HEIGHT - 1) // 10 * 10)
        self.fps_controller = pygame.time.Clock()

    def show_score(self):
        s_font = pygame.font.SysFont('monaco', 24)
        s_surf = s_font.render('Score: {0}'.format(self.score), True, (0, 0, 0))
    ...
    def pudge(self):
        ...
        if self.fruit.check(head.get_x(), head.get_y()):
            self.fruit(randint(0, WIDTH - 1) // 10 * 10,
                       randint(0, HEIGHT - 1) // 10 * 10)
            self.score += 1
